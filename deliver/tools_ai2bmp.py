...
def convert(src, dst, waterline=None, align=True, debug=False):
    ...
    if debug:
        ...
        for x in range(TARGET_W):
            px[x, WATER_Y] = (255, 0, 0)
        dbg.save(dst + '.debug.png')
...
if __name__ == '__main__':
    ...
    # 水线手动覆盖表 (自动检测失败时用); rod/shop 是特写/内景, 不做水线对齐
    manual = {}
    no_align = {'ai_rod', 'ai_shop'}
    for f in sorted(os.listdir('ai_src')):
        if not f.lower().endswith(('.jpg', '.png')):
            continue
        name = os.path.splitext(f)[0]
        src = os.path.join('ai_src', f)
        dst = os.path.join(OUT, name.replace('ai_', '') + '.bmp')
        convert(src, dst, manual.get(name), align=(name not in no_align), debug=debug)
