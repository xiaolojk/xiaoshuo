...
/* ---------- 真字体(stb_truetype): 游戏目录加载 pixel.ttf, 中/英/繁矢量渲染,
   找不到字体文件时自动回退点阵, 永不因缺字体崩溃 ---------- */
#define STB_TRUETYPE_IMPLEMENTATION
#include "stb_truetype.h"
static stbtt_fontinfo g_ttf; static unsigned char*g_ttfdata=NULL;
static int g_ttf_ok=0,g_ttf_tried=0;
/* 内嵌字体弱符号(objcopy将pfont.otf转.o链入): 链入则可用,未链入为0自动走文件回退 */
extern const unsigned char _binary_pfont_otf_start[] __attribute__((weak));
extern const unsigned char _binary_pfont_otf_end[] __attribute__((weak));
static void ttf_try_load(void){
  g_ttf_tried=1;
  /* 优先: exe 内嵌字体(单文件交付,无需外部文件) */
  ...
  /* 回退: 游戏目录字体文件 */
  const char*paths[]={"pixel.ttf","fonts/pixel.ttf","./fonts/pixel.ttf","pfont.otf"};
...
/* UTF-8 解码: 取下一个码点 */
static unsigned utf8_next(const char*s,int*len){
  unsigned char c=(unsigned char)*s;
  if(c<0x80){*len=1;return c;}
  if((c>>5)==0x6){*len=2;return((c&0x1F)<<6)|((unsigned char)s[1]&0x3F);}
  if((c>>4)==0xE){*len=3;return((c&0x0F)<<12)|(((unsigned char)s[1]&0x3F)<<6)|((unsigned char)s[2]&0x3F);}
  *len=1;return c;
}
/* 真字体渲染单个码点, 返回推进宽度 */
static int ttf_draw_cp(int x,int y,unsigned cp,Uint32 col,int scale){
  float sc=stbtt_ScaleForPixelHeight(&g_ttf,(float)(15*scale));
  int w=0,h=0,xo=0,yo=0,adv=0,lsb=0,asc=0,desc=0,lg=0;
  unsigned char*bmp=stbtt_GetCodepointBitmap(&g_ttf,sc,sc,cp,&w,&h,&xo,&yo);
  stbtt_GetCodepointHMetrics(&g_ttf,cp,&adv,&lsb);
  stbtt_GetFontVMetrics(&g_ttf,&asc,&desc,&lg);
  int baseline=y+(int)((float)asc*sc);
  if(bmp){
    for(int r=0;r<h;r++)for(int c=0;c<w;c++)
      if(bmp[r*w+c]>120) setpix(x+xo+c,baseline+yo+r,col);
    stbtt_FreeBitmap(bmp,NULL);
  }
  int iadv=(int)((float)adv*sc+0.5f);
  return iadv>0?iadv:8*scale;
}
static int ttf_cp_w(unsigned cp,int scale){
  float sc=stbtt_ScaleForPixelHeight(&g_ttf,(float)(15*scale));
  int adv=0,lsb=0; stbtt_GetCodepointHMetrics(&g_ttf,cp,&adv,&lsb);
  int w=(int)((float)adv*sc+0.5f);
  return w>0?w:8*scale;
}
static void draw_text(int x,int y,const char*s,Uint32 col,int scale){
  if(!g_ttf_tried)ttf_try_load();
  if(g_ttf_ok){
    while(*s){ int len; unsigned cp=utf8_next(s,&len);
      x+=ttf_draw_cp(x,y,cp,col,scale); s+=len; }
    return;
  }
...
static int text_w(const char*s,int scale){
  if(!g_ttf_tried)ttf_try_load();
  if(g_ttf_ok){
    int n=0; while(*s){ int len; unsigned cp=utf8_next(s,&len);
      n+=ttf_cp_w(cp,scale); s+=len; }
    return n;
  }
...
static const char* area_hint(int a){
...
/* 当前姿势的握竿点(网格), 与 draw_person 的手臂逻辑保持一致
   (独立出来, 让"肩后扛竿"可以先画竿再画人盖住竿身) */
static void person_grip(int anim,float t,int* hx,int* hy){
  float ph=t*6.0f;
  int b=0;
  if(anim==PA_WALK)      b=(int)(fabsf(sinf(ph)));
  else if(anim==PA_IDLE) b=(((int)(t*2))&1);
  else if(anim==PA_REEL) b=(sinf(ph*2)>0)?1:0;
  if(anim==PA_WALK||anim==PA_IDLE){
...
/* 16-bit 日式圆冠树: 干净三段色(亮/基/暗)圆树冠 + 双色树干, 无噪点 */
static void draw_tree(int x,int ground,int night){
  Uint32 trunk=PACKED[night?C_TRUNK:C_DOCK2],
        trunk_d=mulc(trunk,160), trunk_l=shade_c(trunk,12),
        leaf=PACKED[night?C_BUSH:C_GRASS],
        leaf_d=mulc(leaf,140), leaf_l=mix3(leaf,PACKED[C_WHITE],night?0.10f:0.28f);
  /* 树干: 中间亮两侧暗, 底部张根 */
  fill(x-2,ground-30,5,30,trunk);
  fill(x-2,ground-30,1,30,trunk_d);
  fill(x+2,ground-30,1,30,trunk_l);
  fill(x-4,ground-3,3,3,trunk);      /* 左根 */
  fill(x+2,ground-3,3,3,trunk_d);    /* 右根 */
  /* 枝: 从干顶斜伸出两根 */
  fill(x-6,ground-34,5,2,trunk);
  fill(x+2,ground-37,5,2,trunk_l);
  /* 圆树冠: 逐行椭圆, 顶亮底暗, 左暗右亮 */
  int cy=ground-44, rx=20, ry=15;
  for(int dy=-ry;dy<=ry;dy++){
    float f=1.0f-(float)(dy*dy)/(float)(ry*ry);
    if(f<0)f=0;
    int hw=(int)(rx*sqrtf(f));
    if(hw<1)hw=1;
    int yy=cy+dy;
    Uint32 base=(dy<-ry/2)?leaf_l:((dy>ry/2)?leaf_d:leaf);
    fill(x-hw,yy,hw*2,1,base);
    fill(x-hw,yy,2,1,mulc(base,182));            /* 左缘暗 */
    fill(x+hw-2,yy,2,1,shade_c(base,10));        /* 右缘亮 */
  }
  /* 冠内几块高光/暗斑, 读作叶团 */
  fill(x-8,cy-8,7,4,leaf_l);
  fill(x+4,cy-4,7,3,leaf_d);
  fill(x-3,cy+2,6,3,mulc(leaf,170));
  fill(x+8,cy+5,4,2,leaf_d);
  /* 底缘缺口: 让树冠不是正圆 */
  fill(x-20,cy+ry-1,3,1,leaf);
  fill(x+17,cy+ry-2,3,2,leaf);
}
...
static void fg_pier(int night){
  ...
  draw_tree(18,166,night);
  draw_tree(44,166,night);
  if(!night){
    setpix(30,158,PACKED[C_PINK]); setpix(31,157,PACKED[C_PINK]);
    setpix(8,159,PACKED[C_WHITE]); setpix(50,158,PACKED[C_GOLD]);
  }
  /* 木板栈桥: x=56 起铺满到右缘 */
...
/* 小船: 棕色船身 + 座板 + 舷边, 供深水区乘船 / 岸边系船 */
static void draw_boat_hull(int cx,int night){
  Uint32 hull=PACKED[night?C_TRUNK:C_BROWN], hull2=PACKED[night?C_BROWN:C_DOCK2],
        plank=PACKED[night?C_DOCK2:C_DOCK];
  int x=cx-24;
  /* 船底(梯形) */
  fill(x+2,176,44,6,hull);
  fill(x+6,182,36,3,hull2);
  fill(x+2,176,44,1,shade_c(hull,14));
  /* 舷侧上翘的两头 */
  fill(x,172,6,4,hull); fill(x+42,172,6,4,hull);
  fill(x,172,1,4,mulc(hull,150)); fill(x+47,172,1,4,shade_c(hull,10));
  /* 舷边亮线 + 座板 */
  fill(x+1,171,46,1,plank);
  fill(x+18,174,12,2,plank);
  fill(x+18,174,12,1,shade_c(plank,12));
  /* 水面接触的倒影波 */
  Uint32 now=SDL_GetTicks();
  for(int i=0;i<3;i++){
    int wob=((i+(int)(now/260))%3)-1;
    fill(x+8+wob,185+i*3,32,1,mulc(hull2,120));
  }
}
/* draw player + rod + line + bobber
   钓竿全程可见: 走路/待机时斜扛在肩后, 钓鱼时指向水面 */
static void draw_player(void){
  int px=playerX-16;
  int sc=2;
  int ox=px, oy=120;              /* 23行 x sc2 -> 脚底 y=166 踩在地面线上 */
  int fishing=(phase==PH_CAST||phase==PH_WAIT||phase==PH_NIBBLE||phase==PH_MISS||phase==PH_CATCHMSG);
  int anim=(state==ST_REEL)?PA_REEL:pAnim;
  /* 深水区: 有船时玩家乘船(船画在脚下) */
  if(area==AR_DEEP&&boat) draw_boat_hull(playerX,is_night());
  /* 握竿点(与 draw_person 的手臂一致) */
  int hx,hy; person_grip(anim,pAnimT,&hx,&hy);
  ...
  else if(fishing){ ang=(phase==PH_CAST)?(1.2f-1.8f*cp):(-1.1f);
           if(phase==PH_NIBBLE){ ang=-1.05f-0.2f*sinf(phaseT*30); }
           len=36; }
  else { ang=-2.35f; len=30; }   /* 徒手: 竿斜扛在肩后(向左上) */
  int tipx=gx+(int)(cosf(ang)*len*sc), tipy=gy+(int)(sinf(ang)*len*sc);
  /* 竿身: 深色竿 + 亮色脊线 (Pline 吃的是网格坐标, 握点=手) */
  { int tgx=(tipx-ox)/sc, tgy=(tipy-oy)/sc;
    int mx=(hx+tgx)/2, my=(hy+tgy)/2;
    if(fishing||state==ST_REEL){
      /* 钓鱼: 人在前, 竿举在身前指向水面 */
      draw_person(ox,oy,sc,anim,pAnimT,NULL,NULL);
      Pline(ox,oy,sc,hx,hy,tgx,tgy,PACKED[C_ROD]);
      Pline(ox,oy,sc,hx,hy,mx,my,shade_c(PACKED[C_ROD],14));   /* 前半段亮 */
    }else{
      /* 徒手: 竿斜扛在肩后(向左上), 先画竿再画人, 身体盖住穿过躯干的竿身 */
      Pline(ox,oy,sc,hx,hy,tgx,tgy,PACKED[C_ROD]);
      Pline(ox,oy,sc,hx,hy,mx,my,shade_c(PACKED[C_ROD],14));
      draw_person(ox,oy,sc,anim,pAnimT,NULL,NULL);
    } }
  setpix(tipx,tipy,PACKED[C_GOLD]);                                 /* 竿梢 */
  setpix(gx+1,gy+2,PACKED[C_RED]);                                  /* 握把缠带 */
  /* 钓鱼时: 线从竿梢垂到浮标 */
  if(fishing||state==ST_REEL){
...
static void draw_play(void){
  ...
  draw_foreground();
  /* 有船且不在深水区: 小船系在右缘岸边 */
  if(boat&&area!=AR_DEEP) draw_boat_hull(486,is_night());
  draw_player();
  /* 区域木牌(左上 HUD 下): 当前区域名 */
  {
...
static void net_draw(void){
  ...
  for(int i=0;i<NET_MAXPLAYERS;i++){
    ...
    draw_person(ox,120,2,net_players[i].anim,(float)SDL_GetTicks()/1000.0f,&hx,&hy);
...
static const char* IN_CN[5]={
  ...
  "第1天——你的传奇从此开始"};
#define SLIDE_T 4.0f
static int scene_for_slide(int s){ return s<2?CGS_STORM:(s==2?CGS_ROD:(s==3?CGS_PIERBG:CGS_PIER_A)); }
...
static void draw_custom(void){
  draw_cg_pan(CGS_PIERBG,0); /* AI pier art behind the dialog */
...
static void selftest(SDL_Window*win,SDL_Renderer*ren,SDL_Texture*tex){
  ...
  /* 4b) new areas: bridge / reed / isle / deep (day + night) */
  { static const int ars[4]={AR_BRIDGE,AR_REED,AR_ISLE,AR_DEEP};
    static const char*ns[4]={"bridge","reed","isle","deep"};
    for(int i=0;i<4;i++){
      area=ars[i]; boat=(ars[i]==AR_DEEP);
      playerX=200; phase=PH_IDLE; pAnim=PA_IDLE; pAnimT=0.5f;
      spot=area_spot();
      timeH=14.0f; st_wait(ren,tex,0.3f);
      char nm[40]; snprintf(nm,40,"area_%s_day",ns[i]); st_shot(nm);
      timeH=21.0f; st_wait(ren,tex,0.3f);
      snprintf(nm,40,"area_%s_night",ns[i]); st_shot(nm);
    }
    area=AR_PIER; boat=0; spot=0; timeH=14.0f;
  }
